...
const Footer = ({ onRegisterVendor }: Props) => (
  <footer className="bg-neutral-900 dark:bg-black text-stone-100 dark:text-stone-300 w-full relative">
    <div className="w-full px-12 py-24 flex flex-col gap-12 max-w-[1920px] mx-auto">
      <motion.div
        initial={{ opacity: 0, y: 30 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
        transition={{ duration: 1.4 }}
        className="rounded-xl border border-stone-800 px-10 py-12 flex flex-col md:flex-row md:items-center md:justify-between gap-8"
      ...
      <div className="grid grid-cols-12 gap-12">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 1.4 }} className="col-span-12 lg:col-span-4">
      ...
      <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 1.6, delay: 0.8 }} className="pt-24 border-t border-stone-800 flex flex-col md:flex-row justify-between items-center gap-8">
